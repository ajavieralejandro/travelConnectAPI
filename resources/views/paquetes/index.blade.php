    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div class="flex flex-col overflow-hidden transition-shadow duration-300 bg-white shadow-lg paquete-regular rounded-xl hover:shadow-xl">
            <div class="relative aspect-video">
                <img
                    src="{{ $paquete->imagen_principal ?? 'https://via.placeholder.com/400x300' }}"
            ...
            <div class="flex-1 p-6">
                <div class="mb-4">
                    <h2 class="text-xl font-bold text-gray-900 truncate">{{ $paquete->nombre }}</h2>
                    <p class="mt-1 text-sm text-gray-500">
                        <span class="font-medium">{{ $paquete->ciudad }}</span> - {{ $paquete->pais }}
                    </p>
                </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center text-sm text-gray-500">
                        ...
                        {{ $paquete->fecha_vigencia_desde->format('d M') }} - {{ $paquete->fecha_vigencia_hasta->format('d M Y') }}
                    </div>
                    ...
                    <div class="flex items-center text-sm text-gray-500">
                        ...
                        {{ $paquete->tipo_producto }} • {{ $paquete->transporte }}
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between mt-6">
                    <div>
                        <span class="text-2xl font-bold text-gray-900">
                            {{ $paquete->tipo_moneda }} {{ number_format($paquete->precio, 2) }}
                        </span>
                        @if($paquete->descuento > 0)
                            <span class="ml-2 text-sm text-gray-400 line-through">
                                {{ $paquete->tipo_moneda }} {{ number_format($paquete->precio_original, 2) }}
                            </span>
                        @endif
                    </div>

                    <span class="px-3 py-1 text-sm font-semibold rounded-full {{ $paquete->activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        {{ $paquete->activo ? 'Disponible' : 'Agotado' }}
                    </span>
                </div>
        ...
        <!-- Paquetes Julia -->
        @foreach($tarjetasJulia as $paquete)
        <div class="transition-shadow duration-300 bg-white shadow-lg paquete-julia rounded-xl hover:shadow-xl">
            <div class="relative h-48 overflow-hidden rounded-t-xl">
                <img
                    src="{{ $paquete->imagen_principal ?? 'https://images.unsplash.com/photo-1544551763-46a013bb70d5?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80' }}"
                    alt="{{ $paquete->nombre }}"
                    class="object-cover w-full h-full"
                    loading="lazy"
                >
            ...
            <div class="p-6">
                <div class="mb-4">
                    <h2 class="text-xl font-bold text-gray-800 truncate">{{ $paquete->nombre }}</h2>
                    <div class="flex items-center mt-2 text-sm text-gray-500">
                        <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        </svg>
                        {{ $paquete->ciudad }} ({{ $paquete->ciudad_iata }}), {{ $paquete->pais }}
                    </div>
                </div>
                ...
                <div class="mb-4 space-y-2">
                    ...
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        {{ $paquete->cant_noches }} noches
                    </div>
                </div>

                <div class="flex flex-wrap gap-2 mb-4">
                    @foreach($paquete->componentes as $componente)
                    <span class="px-2 py-1 text-xs text-blue-800 bg-blue-100 rounded-full">
                        {{ $componente }}
                    </span>
                    @endforeach

                    @foreach($paquete->categorias as $categoria)
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <span class="text-sm font-semibold text-gray-600">{{ $paquete->tipo_producto }}</span>
                        <p class="text-sm text-gray-500">{{ $paquete->transporte }}</p>
                    </div>
